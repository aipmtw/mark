...
function Auth() {
  ...
  async function handleGitHubLogin() {
    await supabase.auth.signInWithOAuth({
      provider: 'github',
      options: { redirectTo: window.location.origin + window.location.pathname + '#dailynotes' }
    })
  }
...
function NoteEditor({ user, date, existingNote, onSaved }) {
  ...
  async function save() {
    ...
    setMsg('')
    const noteData = {
      user_id: user.id, date, content: content.trim(), visibility,
      repo_id: REPO_ID, updated_at: new Date().toISOString()
    }
    let result
    if (existingNote) {
...
export default function DailyNotes({ onBack }) {
  ...
  const displayName = user?.user_metadata?.display_name || user?.user_metadata?.user_name || user?.email?.split('@')[0]
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white flex flex-col">
      <nav className="max-w-2xl mx-auto px-6 py-6 w-full flex items-center justify-between">
        <button onClick={onBack} className="text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 px-3 py-1.5 rounded-lg transition">
          &larr; 返回首頁
        </button>
        {user && (
